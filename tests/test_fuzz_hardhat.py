...
def setup_hardhat_project(base_path, switch_dir=False):
    # switch to temp dir if requested
    if switch_dir:
        os.chdir(str(base_path))

    # add hardhat project structure
    os.makedirs(str(base_path / "artifacts/contracts/MasterChefV2.sol/"))
    os.makedirs(str(base_path / "artifacts/build-info/"))
    os.makedirs(str(base_path / "contracts"))

    # add sample brownie artifact
    with open(base_path / "artifacts/build-info/b78e6e91d6666dbbf407d4a383cd8177.json", "w+") as artifact_f:
        json.dump(HARDHAT_BUILD_INFO_ARTIFACT, artifact_f)

    with open(base_path / "hardhat.config.ts", "w+") as config_f:
        json.dump("sample", config_f)

    for filename, content in HARDHAT_ARTIFACT.items():
        with open(base_path / f"artifacts/contracts/MasterChefV2.sol/{filename}.json", "w+") as sol_f:
            json.dump(content, sol_f)

    with open(base_path / "contracts/MasterChefV2.sol", "w+") as sol_f:
...
def test_fuzz_no_contract_at_address(tmp_path):
    setup_hardhat_project(tmp_path, switch_dir=False)
...
def test_faas_not_running(tmp_path):
    ...
    with patch.object(
        ...
        runner = CliRunner()
        result = runner.invoke(cli, ["fuzz", "run", f"{tmp_path}/contracts/MasterChefV2.sol"])
