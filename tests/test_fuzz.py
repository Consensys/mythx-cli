...
def test_fuzz_no_build_dir(tmp_path):
    ...
    assert "Error: build_directory not found on .mythx.yaml config file" in result.output
...
def test_fuzz_no_deployed_address(tmp_path):
    ...
    result = runner.invoke(cli, ["fuzz", "run", "contracts"])
    assert "Error: deployed_contract_address not found on .mythx.yaml config file." in result.output
    assert result.exit_code != 0
...
def test_fuzz_no_contract_at_address(tmp_path):
    setup_brownie_project(
        tmp_path, compiled=False, switch_dir=False
    )

    with open(".mythx.yml", "w+") as conf_f:
        conf_f.write(generate_config_file(base_path=tmp_path))

    with patch.object(RPCClient, 'contract_exists') as contract_exists_mock, patch.object(
        RPCClient, 'get_all_blocks') as get_all_blocks_mock:
        get_all_blocks_mock.return_value = get_test_case("testdata/ganache-all-blocks.json")
...
def test_faas_not_running(tmp_path):
    ...
    with patch.object(RPCClient, 'contract_exists') as contract_exists_mock, patch.object(
        RPCClient, 'get_all_blocks') as get_all_blocks_mock, patch.object(
        FaasClient, 'start_faas_campaign') as start_faas_campaign_mock:
        get_all_blocks_mock.return_value = get_test_case("testdata/ganache-all-blocks.json")
        contract_exists_mock.return_value = True
        start_faas_campaign_mock.side_effect = RequestError(f"Error starting FaaS campaign.")

        runner = CliRunner()
        result = runner.invoke(cli, ["fuzz", "run", f"{tmp_path}/contracts"])

    assert "Error: Unable to submit the campaign to the faas. Are you sure the service is running on" in result.output
    assert result.exit_code != 0
...
def test_fuzz_run(tmp_path, keyword):
    ...
    contract_exists_mock.assert_called_with("0x7277646075fa72737e1F6114654C5d9949a67dF2")
    ...
    assert f"You can view campaign here: {FAAS_URL}/campaigns/{campaign_id}" in result.output

    request_payload = json.dumps(called_with.args[0])
